...
const submitApplication = async (req, res) => {
    try {
        ...
        await triggerNotification({
            ...
            emailMessage: `Hi ${req.user.name},\n\nYour application for ${course.name} has been received and is now pending review.\n\nBest regards,\nAdmissions Team`
        });
...
const updateApplicationStatus = async (req, res) => {
    try {
        ...
        // Trigger Dynamic Status Notifications (Verified, Admitted, Rejected)
        const studentUser = await User.findById(application.applicantId);
        if (studentUser) {
            let notificationType = 'status_update';
            let emailSubject = 'ScholarStack Application Update';
            let emailMessage = `Hi ${studentUser.name},\n\nYour application status has been updated to: ${status}.\n\nRemarks: ${remarks || 'None'}\n\nBest regards,\nAdmissions Team`;
            
            if (status === 'verified') {
                notificationType = 'verification_completion';
                emailSubject = 'Application Documents Verified';
                emailMessage = `Hi ${studentUser.name},\n\nYour application documents have been successfully verified.\n\nBest regards,\nAdmissions Team`;
